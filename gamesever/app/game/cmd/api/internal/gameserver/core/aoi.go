...
func NewAOIManager(roomId int64, minX, maxX, cntsX, minY, maxY, cntsY int) *AOIManager {
	aoiMgr := &AOIManager{
		RoomId: roomId,
		MinX:   minX,
		MaxX:   maxX,
...
func (m *AOIManager) GetPIDsByPos(x, y float32) (playerIDs []int64) {
...
func (m *AOIManager) GetPIDsByGID(gID int) (playerIDs []int64) {
...
// 移除一个格子中的PlayerID
func (m *AOIManager) RemovePIDFromGrID(pID int64, gID int) {
	m.grIDs[gID].Remove(pID)
}

// AddPIDToGrID Add a PlayerID to a grid
// 添加一个PlayerID到一个格子中
func (m *AOIManager) AddPIDToGrID(pID int64, gID int) {
	m.grIDs[gID].Add(pID)
}

// AddToGrIDByPos Add a Player to a grid based on horizontal and vertical coordinates
// 通过横纵坐标添加一个Player到一个格子中
func (m *AOIManager) AddToGrIDByPos(pID int64, x, y float32) {
	gID := m.GetGIDByPos(x, y)
	grID := m.grIDs[gID]
	grID.Add(pID)
}
...
func (m *AOIManager) RemoveFromGrIDByPos(pID int64, x, y float32) {
	gID := m.GetGIDByPos(x, y)
	grID := m.grIDs[gID]
	grID.Remove(pID)
}
