...
func InitObj(addr string, pwd string, db int) {
	once_MyCache.Do(func() {
		...
		obj_MyCache.CacheTimes = 30 * time.Minute // 30分钟
		//obj_MyCache.CacheTimes = 2 * time.Hour // 2 小时
		//可连接性检测
		_, err := client.Ping().Result()
		if err != nil {
			zlog.Errorf("redis链接错误 ,err =  %s\n", err.Error())
			fmt.Printf("redis链接错误 ,err =  %s\n", err.Error())
			return
		}
		fmt.Print("redis链接成功  addr =" + addr)
...
// 	// if err != nil {
// 	// 	//zlog.Error("redis connect ping failed, err:", zap.Any("err", err))
// 	// } else {
// 	// 	//zlog.Error("redis connect ping response:", zap.String("pong", pong))
// 	// 	m.MyRides = client
// 	// }
// }
...
func (m *MyCache) Set(key string, value interface{}, second int) error {
	...
	if cmd.Err() != nil {
		zlog.Errorf(" 缓存设置出错 %s", cmd.Err().Error())
		//panic(err)
	}
	return cmd.Err()
}
...
func (m *MyCache) SetObj(key string, value interface{}, second int) error {
	// 将 value 序列化为 bytes
	//zlog.Error(" SetObj cacheKey  =" + key)
	//转成byte[]
...
func (m *MyCache) GetObj(key string) (interface{}, error) {
	...
	zlog.Error("GetObj cacheKey  =" + key)
	return value, err
}
...
func (m *MyCache) DeleteKeyPre(prefix string) (num int, err error) {
	...
	if num > 0 {
		...
		for _, key := range keysToDelete {
			zlog.Errorf(" 批量删除 key =%s ", key)
			pipeline.Del(key)
		}
		_, err = pipeline.Exec()
		if err != nil {
			zlog.Errorf(" 批量删除 err =%s ", err.Error())
			return 0, err
		}
	}
	return num, nil
