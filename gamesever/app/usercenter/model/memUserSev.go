...
func NewMemUserSev(db *gorm.DB, rds *redis.Redis) *MemUserSev {
...
func (m *MemUserSev) GetByUsername(ctx context.Context, username string) (obj MemUser, err error) {
	cacheKey := m.GetCacheKey("MemUser_name", username)
	if cacheData, err := m.Rds.GetObj(cacheKey); err == nil {
...
func (m *MemUserSev) Create(ctx context.Context, data MemUser) (id int64, err error) {
	...
	if err == nil {
		m.Rds.DeleteKeyPre(m.CacheKeyListPre)
		return data.Id, err
	} else {
		return 0, err
	}
...
func (m *MemUserSev) Get(ctx context.Context, id int64, fields string) (obj MemUser, err error) {
	...
	if cacheData, err := m.Rds.GetObjCtx(ctx, cacheKey); err == nil {
		fmt.Println("从缓存获取。。。。obj.cacheKey = ", cacheKey)
		obj = cacheData.(MemUser)
		fmt.Println("从缓存获取。。。。obj.Username ", obj.Username)
		return obj, err
	} else {
		if err != nil {
			fmt.Println("redis 错误 ", err.Error())
		}
	}
	fmt.Println("从数据库 获取。。。。obj.Username ", obj.Username)
	...
	if err == nil {
		m.getFile(ctx, &obj)
		err = m.Rds.SetObjCtx(ctx, cacheKey, obj, -1)
		if err != nil {
			fmt.Println("redis 错误22222 ", err.Error())
		}
	}
	return obj, nil
}
